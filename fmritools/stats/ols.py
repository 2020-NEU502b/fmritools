...
class OLS(object):
    ...
    def f_test(self, contrast, pvalues=True):
        ...
        if np.ndim(contrast) == 1: 
            C = np.insert(np.zeros((self.n_coef-1, self.n_coef)), 0, contrast, 0)
        ...
        ## Error-catching.
        np.testing.assert_array_equal(C.shape, (self.n_coef, self.n_coef))
        ...
        for i, (cw, cov) in enumerate(zip(Cw.T, Sigma)):
            F[i] = cw.T @ np.linalg.pinv(rank * C @ cov @ C) @ cw
